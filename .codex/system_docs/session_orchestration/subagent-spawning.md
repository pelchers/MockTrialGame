...
1. **Read** the queue file from `.codex/orchestration/queue/next_phase.json`.
...
3. **Build** the `claude exec` command:
   - If `agent` is set, prefix the prompt with the agent file path
     (e.g., `--agent .codex/agents/longrunning-worker-subagent/AGENT.md`).
...
`.codex/skills/orchestrator-session/templates/poke_template.md`
