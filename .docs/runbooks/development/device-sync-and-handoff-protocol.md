...
## 2b. IDLE AUTO-HANDOFF — the safety net (no manual `/winddown` needed)

For the real-world case where you just **walk away and switch machines**, a background monitor
(`.claude/hooks/scripts/idle-handoff-monitor.sh`) does an automatic partial handoff:

- Scheduled every ~20 min via **`register-idle-handoff.ps1`** (a Windows Scheduled Task — run it once
  per machine per repo; it is NOT a Claude event hook, which can't fire on a timer).
- After **`IDLE_HANDOFF_HOURS`** (default 4) of **no source-file edits**, if there's un-handed-off work
  it: commits it → **prepends an AUTO-HANDOFF `HANDOFF.md` entry** → **pushes the DEVICE BRANCH**.
- It **never syncs `main`** (main = stable/prod — auto-pushing mid-work code there is unsafe). It
  single-fires per work-batch (`.git/idle-handoff-state`), never force-pushes, resolves the lane from
  `device.local.md`, and notes if `.chat-history` looks stale.
- Config: `IDLE_HANDOFF=off` disables · `IDLE_HANDOFF_HOURS=<n>` · `IDLE_HANDOFF_AGENT=1` (richer
  summary via the subscription CLI).

**Net effect:** switch machines and just tell the new agent *"I'm back — what's the state of things?"*
It runs PICKUP, pulls your auto-pushed device branch, reads the newest `HANDOFF.md` entry, and resumes.
Nothing is lost even if you never ran `/winddown`. (A deliberate `/winddown` is still how you sync
`main`/prod when the work is actually done.)
...
| **idle monitor** | `.claude/hooks/scripts/idle-handoff-monitor.sh` (+ `.codex/`) | Background: after N idle hours, auto-commit + HANDOFF + push the device branch (never main). See §2b. |
| **monitor setup** | `.claude/hooks/scripts/register-idle-handoff.ps1` (+ `.codex/`) | Registers the monitor as a Windows Scheduled Task (once per machine). |
