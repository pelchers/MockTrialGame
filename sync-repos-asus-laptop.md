...
| `contributor-conventions` | `.other-devices/components/contributor-conventions/` | (home-desktop, built on CC-Local) — staged; propagation to SYNC-REPOS targets pending; two hooks left to orchestrator | 2026-07-22 |
...
| 2026-07-22 | **NEW component `contributor-conventions`** (contributor-vs-owner repo model: `repo.local.md` declaration + template + skill + `/promote-pr` + `/declare-repo` + `contributor-promotion-agent` + two **ORCHESTRATOR-OWNED** hooks [`contributor-repo-check.sh` SessionStart, `contributor-push-guard.sh` PreToolUse fail-open — **spec only, NOT built**] + runbook + system_docs [README/USAGE_GUIDE/ARCHITECTURE/HOOKS-TODO] + staged package + one-line governance pointer at the END of the `production-pr-on-request` block in CLAUDE.md/.claude/CLAUDE.md/.codex/CODEX.md/.codex/AGENTS.md/git-workflow-agent). Built on **CC-Local (home-desktop)**; **staged only — NOT yet propagated** to SYNC-REPOS targets and **not committed** (per task scope; orchestrator owns the two hook scripts + settings.json registration). | (staged on CC-Local; propagation pending) | files present on CC-Local; `.claude`⇄`.codex` mirrored; settings.json + hook scripts intentionally untouched |
